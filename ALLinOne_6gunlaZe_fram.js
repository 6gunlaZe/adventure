...
//game_log("characterRange" +monsterscharacterRange.length)		
            ...
            if( (leader && leader.hp < 8000) || (healerr && healerr.hp < 5000)  ){
		weaponSet("heal");
...
if (character.map == "cave" && distance(character, {x: -194, y: -1281}) > 30)smart_move({ map: "cave", x: -194, y: -1281 })
	...
	if(!currentTarget)
	{
		var currentTarget1 = get_nearest_monster_solobosskill()  ////đối tượng tổng
		var currentTargeta5 = get_nearest_monster_solobosskilla5()  ///đối tượng a5
		var currentTarget11 = get_nearest_monster_solobosskill1() ///đối tượng cũ sẽ chắc chăn khong có a5
		...
		if(currentTargeta5) {
		 ...
		 {
                 if (currentTarget11 && is_in_range(currentTarget11, "supershot") && character.mp > 500 && currentTarget11.hp >10000  && !is_on_cooldown("supershot") && Date.now() > delayboss + 10000 ) {
                delayboss = Date.now()
                use_skill("supershot", currentTarget11);
                game_log("Supershot!!");
                                    }
		...
		else if(currentTarget1 && !currentTargeta5) {
...
setInterval(function() {
    ...
    let itemsToExclude = ["mittens","supermittens","snowball","hpot0", "mpot0","hpot1", "mpot1", "elixirint0","elixirstr0","elixirdex0","elixirint1","elixirstr1","elixirdex1", "luckbooster", "goldbooster", "xpbooster", "pumpkinspice", "xptome","cscroll0", "cscroll1", "scroll0", "scroll1", "tracker","crossbow","jacko", "pouchbow","orbg","cupid"];
...
////
function get_nearest_monster_solobosskill1(args) ///mod
{
	//args:
	// max_att - max attack
	// min_xp - min XP
