...
var craftList = [ "cryptkey","essenceofgreed","sshield","vdagger","vhammer","scythe","bataxe","rabbitsfoot","bfangamulet","suckerpunch","cdarktristone","powerglove","xarmor","harmor","tshirt9","fury","xhelmet","hhelmet","lostearring", "mittens", "glolipop" , "ololipop" , "supermittens" , "sweaterhs" , "xhelmet" , "xgloves" , "frankypants" , "starkillers" , "wbookhs", "basher"  , "crossbow" , "harbringer",  "mcape" , "hgloves" , "hboots", "t2quiver" , "gem0" , "ornament" , "candycane" , "mistletoe" , "seashell"  ];
...
setInterval(function() {
    ...
    for (let i = 0; i < 42; i++) {
        ...
        if (item1 && itemsToExclude1.includes(item1.name) && !item1.l && !item1.s) {
            //send_item(lootMule.id, i, item.q ?? 1);
			if (lootMule != null&& vanchuyenHPMP == 1) {
				send_item(lootMule.id, i, 2000);
			}
			if (lootMule2 != null&& vanchuyenHPMP == 2) {
				send_item(lootMule2.id, i, 8000);
			}
			if (lootMule3 != null&& vanchuyenHPMP == 3) {
				send_item(lootMule3.id, i, 2000);
			}
			if (lootMule4 != null && vanchuyenHPMP ==4) {
				send_item(lootMule4.id, i, 2000);
			}	
			if (lootMule5 != null && vanchuyenHPMP ==5) {
				send_item(lootMule5.id, i, 2000);
			}				
...
{
	changeitem({ slot: "orb", name : "orbg", level : 1 });
}
