...
const locations = {
  ...
  { type: "safe", x: -2, y: -1799, map: "winterland" }
    ...
    xscorpion: [
  { type: "farm", x: -523, y: 676 , map: "halloween" },
  { type: "safe", x: -283, y: 731 , map: "halloween" }
    ],
    odino: [
...
if(parent.party_list.includes(Xmagelayer) && (!member1  || character.map != "winter_instance" || get_nearest_monster({ type: home }) ) ){
   ...
   // Chế độ cho thêm pháp sư  
    ...
    // cho dừng luôn
    if ( (boss_fi || (boss_fz && boss_fz.hp < 20) ) && member1 && member1.name == Xmagelayer && character.map === "winter_instance")
	{
...
// --- LOGIC DI CHUYỂN NGOÀI MAP ---> tới vị trí hầm ngục
    ...
    if (character.map === "winter_instance") {
        ...
        if (boss_x || (boss_fn && boss_fn.hp < 450000) ) {
            ...
            if (gen && boss_x && distance(character, boss_x) < character.range) {
...
async function skillLoop() {
    ...
    try {
        ...
        const aoe = character.mp >= character.mp_cost * 2 + G.skills.cleave.mp + 130;
...
function handleWeaponSwap(stMaps, aoeMaps, Mainhand, offhand) {
    ...
    // TỐI ƯU 1: Gom các điều kiện phân loại quái vào 1 lượt duyệt duy nhất
    let physicalCount = 0;
    let magicOrbCount = 0;
    let magicalCount = 0;
    let fireCount = 0;
    ...
    for (const entity of Object.values(parent.entities)) {
        ...
        if (distance(character, entity) <= 120) {
           ...
           if (entity.target) Soluongquai++;
            ...
            if (isTarget || isCoop) {
                // Check Coop Mob HP
                if (isCoop && entity.hp != null && entity.hp < 180000) {
                    hasLowHP_CoopMob = true;
                }

                // Check Fire Mob
                if (entity.mtype === "xmagefi") fireCount++;
                ...
                if (entity.mtype !== "crabxx" && entity.mtype !== "ent" && entity.damage_type === "physical") {
                    if ((character.hp < 8000 && entity.attack > 1000) ||
                        (character.hp < 15000 && entity.attack > 2000) ||
                        (entity.attack > 3500)) {
                        physicalCount++;
                    }
                }
                ...
                // Check Magical
                if (entity.damage_type === "magical") {
                    if ((character.hp < 12000 && entity.attack > 1000) ||
                        (character.hp < 15000 && entity.attack > 2000) ||
                        (entity.attack > 3500)) {
                        magicalCount++;
                    }
...
const targetHighArmor = targetNow?.armor > 300;
    ...
    if (character.hp > 15000 && !hasLowHP_CoopMob && physicalCount === 0 && magicalCount === 0) {
        // Ghi thời điểm bắt đầu an toàn
        if (!defSafeSince) defSafeSince = currentTime;

        // Đủ thời gian an toàn → gỡ phòng thủ / check lại mỗi 40s
        if ((currentTime - defSafeSince >= 3000 && character.slots.helmet?.name === "xhelmet") || (currentTime - defSafeSince >= 40000 && targetNow) ) {
            checkdef = 0;
            defSafeSince = null;
            eTime = currentTime;
...
async function handleWarriorSkills(tank, f1) {
    ...
    for (let id in parent.entities) {
        ...
        // Tìm con quái free pull gần nhất chưa có target
...
if (current.mtype === "ent" && current.target) {
    ...
    // =========================================================================
    // LOGIC XỬ LÝ TAUNT (Thứ tự ưu tiên từ trên xuống dưới)
    // =========================================================================
    ...
    // 🐷 ƯU TIÊN 3: Chủ động Taunt quái trong danh sách Free Pull (Ent, Pppompom,...)
        ...
        (freePullMob.mtype !== "ent" || !character.target)
    ) {
...
// 🌲 ƯU TIÊN 4: Chỉ hút tối đa 2 con Ent từ Ynhi NẾU Warrior chưa bị con Ent nào đánh (checkENT === 0)
else if (
    checkENT <= 1 && character.hp > 15000 && character.mp > 1000 &&
    hutENT && 
    !is_on_cooldown("taunt") && 
    ...
    for (let current of dangerousMobsToTaunt) {
        if (is_in_range(current, "taunt") && !is_on_cooldown("taunt")) {
            ...
            break; // 🛠 SỬA: Thêm break để chỉ taunt 1 con duy nhất trong 1 loop
        }
    }
}
...
function temporalSurgeLogic() {
    ...
    // Tính theo loại quái
    // =========================
    const temporalGap = home === "bscorpion" ? 0 : TEMPORAL_GAP;
    const temporalDelay = home === "bscorpion" ? 0 : TEMPORAL_DELAY;
...
function use_hp_or_mp1() {
    ...
    try {
        ...
        else if (character.hp / character.max_hp < 0.8 && character.mp > 150) {
            skill = "use_hp";
        } 
...
setInterval(function() {
    ...
    let itemsToExclude = ["fieldgen0","frozenkey1","spiderkey","candycanesword","elixirfires","hotchocolate","elixirluck","hboots","cryptkey","hpot0", "mpot0","hpot1", "mpot1", "elixirint0","elixirstr0","elixirdex0","elixirint1","elixirstr1","elixirdex1", "luckbooster", "goldbooster", "xpbooster", "pumpkinspice", "xptome", "jacko","tracker","supermittens","mittens","xgloves","exoarm","hhelmet","fury", "basher","bataxe","tigerstone","rabbitsfoot"];
...
async function handlebossPro(eventType, mapName, x, y, hpThreshold,f1name,f2name) {
    ...
    if (!smart.moving && eventType != "icegolem") {  //chặn lỗi vì không thể đi tới vị trí đảo
    try {
