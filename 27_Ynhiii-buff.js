...
function use_hp_or_mp1() {
    ...
    try {
        ...
        if (character.hp < 2100 && character.mp > 150) {
            ...
            skill = "use_mp";
        } else if (character.mp / character.max_mp < 0.95 && character.targets > 0 ) {
            skill = "use_mp";
...
setInterval(function() {
    ...
    let itemsToExclude = ["fieldgen0","frozenkey","spiderkey","elixirfires","hotchocolate","elixirluck","snowball","supermittens","handofmidas","hpot0", "mpot0","hpot1", "mpot1", "elixirint0","elixirstr0","elixirdex0","elixirint1","elixirint2","elixirstr1","elixirdex1", "luckbooster", "goldbooster", "xpbooster", "pumpkinspice", "xptome", "tracker","tigerstone","froststaff","wbook1"];
...
function tryPartyHeal() {
    ...
    if (is_on_cooldown("partyheal")) return false; // chuyển xuống
...
async function ZapCase() {
    ...
    try {
        if (!character.rip) {
            ...
            if ( Date.now()-lastEntRequest>150000 && isFullTeam && character.hp/character.max_hp > 0.75 && entcheck <= 2 && character.map == "desertland" && !smart.moving && (crepp == "fireroamer" || crepp == "plantoid" ) && character.targets > 6 && !parent.S.franky && !parent.S.crabxx )
			{
				send_cm("MuaBan","ent");
...
async function equipBatch(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        for (let j = 0; j < parent.character.items.length; j++) {
            ...
            if (item && item.name === itemName && item.level === level && (item.l === l || l === undefined) ) {
