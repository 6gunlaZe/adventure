...
// if (check_viem_xung_quanh() == 1) targetedForMoreThanOneSecond = true;  ///chỉ mở lại khi muốn kill a4
    ...
    for (id in parent.entities) {
        ...
        if ((  (current.mtype == 'zapper0' || current.mtype == 'a4' || (current.mtype == 'nerfedbat' && leader && a1 && distance(leader, a1) < 195 ) )  || character.hp < 6000 || (smart.moving && character.map != "crypt") ) && current.target == character.name) {
            mobnum++;
...
function get_nearest_monster_solobosskill(args) ///mod
{
	...
	for(id in parent.entities)
	{
		var current=parent.entities[id];
		if ( (bossarmy.indexOf(current.mtype) == -1)   ) continue
		if ( (bossarmylevel.indexOf(current.mtype) != -1) && current.level > 2  ) continue
		if(current.type!="monster" || !current.visible || current.dead) continue;
		if(args.type && current.mtype!=args.type) continue;
		if(args.min_xp && current.xp<args.min_xp) continue;
		if(args.max_att && current.attack>args.max_att) continue;
...
const avoidTypes = ["a0","a2","a3","a4","a6", "a7","a8","a9","vbat","stompy","skeletor","crabxx"];
