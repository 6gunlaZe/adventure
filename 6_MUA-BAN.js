...
if(["pstem","skullamulet","phelmet","smush","stinger","hpamulet", "hpbelt", "mushroomstaff","frogt" ,"sword","vitearring","hbow","spores", "rattail", "warmscarf", "xmasshoes", "xmaspants", "ornamentstaff","rednose","candycanesword","xmassweater","xmashat","vitring","beewings","crabclaw","santasbelt", "iceskates","wbook0", "swifty", "merry", "hotchocolate","eggnog", "snowflakes","quiver", ].includes(character.items?.[slot]?.name)) sell(slot, character.items?.[slot]?.q ? character.items?.[slot]?.q : 1)	
	}
...
const muleItemsMap = {
    ...
    "Ynhi": ["elixirfires"],
	"haiz": ["tombkey", "cryptkey","spiderkey"],
};
...
////// tu dong vo bank khi tui do day
	if( (character.stand || character.esize < 1  ) && character.map == "main" && (character.esize < 5 || moveBANKStatus == 1 ) && !smart.moving && checktui == 0 && vanchuyen == 0){
		checktui = 1
		smart_move('bank')
	delaybank = Date.now()
    }
...
var craftList = [
    "smoke","throwingstars","weaponbox","intbelt","strbelt",
