...
setInterval(function() {
    ...
    let itemsToExclude = ["elixirfires","hotchocolate","elixirluck","snowball","wbookhs","mittens","handofmidas","hpot0", "mpot0","hpot1", "mpot1", "elixirint0","elixirstr0","elixirdex0","elixirint1","elixirint2","elixirstr1","elixirdex1", "luckbooster", "goldbooster", "xpbooster", "pumpkinspice", "xptome","cscroll0", "cscroll1", "scroll0", "scroll1", "tracker","harbringer","jacko","slimestaff","tigerstone","froststaff","wbook1"];
...
//////////// dung skill
	 	...
	 	var target1= get_nearest_monster({type: "franky",});
		    if(target1&& character.mp > 3500 && !is_on_cooldown("curse") &&  !target1.s["cursed"] )
            {
	 	...
	 	var target2= get_nearest_monster({type: "icegolem",});
		    if(target2&& character.mp > 3500 && !is_on_cooldown("curse") &&  !target2.s["cursed"] )
            {
	 	...
	 	var target3= get_nearest_monster({type: "crabxx",});
		    if(target3&& character.mp > 3500 && !is_on_cooldown("curse") &&  !target3.s["cursed"] )
            {
	...
	var target1xc= get_nearest_monster1({comuctieu: 1 ,  nhonhat: 41000});  //nhonhat : 41000   lonnhat: 1
		    if(target1xc&& character.mp > 3500 && !is_on_cooldown("curse") &&  !target1xc.s["cursed"] && character.map != "winter_instance" && target1xc.hp > 40000 )
            {
		    ...
		    if(currentTarget&& character.mp > 3500 && !is_on_cooldown("curse") && currentTarget.target == "haiz" &&  !currentTarget.s["cursed"] && character.map != "winter_instance" && currentTarget.hp > 30000 )
            {
...
var target11= get_nearest_monster1({type: crepp, subtype: "bigbird" , NO_target: 1});
	...
	if (checkvar && cung1 ){
if (!target1 && character.targets <= 6 && target11 && character.hp/character.max_hp > 0.75 && cung1.hp > 12700 && cung1.mp > 200 && target11.level < 4 ) ////////////tùy chỉnh level theo độ mạnh của quái/////////////////hút quái cho framer
{
