...
if (z > 990)return
if (get_NUMber_kill() >= 15 && targetkill.length == 0 || z > 123 || (checkback + z) > 180 ){
	stop_character("Ynhi")	
	stop_character("6gunlaZe")	
	z = 1000
	monsterIds = [];
...
async function attackLoop() {
    ...
    try {
        let nearest = null;
        var KILLdauTien = getBestTargets({ max_range: character.range, type: "zapper00000", subtype: "a5",  number: 1 }); 
...
const ango  = [ "a2" , "a3", "a7", "vbat","a8","a6","a4","a1","zapper0000"];
for (let id in parent.entities) {
