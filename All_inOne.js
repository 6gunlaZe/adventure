...
async function checkGameEvents() {
    ...
    for (let event of events1) {
		...
		if (server.region == "US" && server.id == "II" )isEventValid = !!parent?.S?.[event.eventType];
    ...
    try {
	    ...
	    var bossarmy=["icegolem", "franky" , "crabxx" ]; 
	    	    var mob=["phoenix", "jr","greenjr", "mvampire","snowman","bgoo","rgoo","wabbit"];
                    var mob2=["stompy", "skeletor","gbluepro","gredpro","ggreenpro", "xmagefz","xmagefi","xmagefn","xmagex", ]; //boss mạnh cần có healter
    ...
    try {
        ...
        if (character.ctype === "warrior") {
            try {
                ...
                if ( character.mp >= 170 && ( (f1 && f1.hp < f1.max_hp * 0.6 )  || (tank && tank.hp < tank.max_hp * 0.6 )  || (tank && tank.mp < 1500 && tank.ctype == "priest" )  || (tank && tank.mp < 5500 && tank.ctype == "priest" && character.mp > character.max_mp * 0.8  ) || character.hp < 12000 ) && character.map != "winter_instance"  ){
...
if ( region == "US" && serverIden == "II" ) 
{
	...
	{
         if(character.hp > 8000)change_server("US", "II");
	//change_server("ASIA", "I");	
	}
}	
...
setInterval(function() {
    ...
    let itemsToExclude = ["elixirfires","hotchocolate","tombkey","elixirluck","candypop","hboots","cryptkey","hpot0", "mpot0","hpot1", "mpot1", "elixirint0","elixirstr0","elixirdex0","elixirint1","elixirstr1","elixirdex1", "luckbooster", "goldbooster", "xpbooster", "pumpkinspice", "xptome","cscroll0", "cscroll1", "scroll0", "scroll1", "jacko","tracker","mittens","xgloves","exoarm","hhelmet","fury","wbasher", "basher","bataxe","sweaterhs","tigerstone","rabbitsfoot"];
...
if ( region11 == "US" && serverIden11 == "II" ) 
{
change_server("ASIA", "I");	
}
	else
